...
Esta seção descreve os procedimentos adotados na implementação da solução proposta, a qual visa automatizar o processo de confecção de atas de reunião no contexto jurídico por meio do uso de modelos de linguagem de código aberto. Os passos aqui apresentados correspondem às etapas que sucederam a definição do método e operacionalizam os requisitos estabelecidos para o sistema. Assim, detalha-se a arquitetura desenvolvida, os módulos funcionais que compõem a solução, bem como o fluxo de processamento responsável por transformar o áudio bruto da reunião em um documento estruturado no formato de ata.

Como etapa preliminar, foi necessário definir o modelo de ata a ser seguido pela aplicação, de modo a assegurar que todas as informações essenciais fossem contempladas. Para isso, adotou-se como referência o padrão utilizado pela equipe do Laboratório LIODS do Tribunal Regional Eleitoral do Ceará, ajustando-o às necessidades do presente estudo. Esse modelo estabelece os elementos mínimos que caracterizam uma ata de reunião institucional, compreendendo: identificação dos participantes, registro das pautas tratadas, síntese das discussões realizadas e descrição das deliberações tomadas. A partir desse conjunto de requisitos, estruturou-se o formato alvo para o documento que o sistema deve produzir automaticamente.
