...
Todos os métodos e \textit{prompts} desenvolvidos foram avaliados utilizando os seguintes modelos de linguagem disponíveis na plataforma Ollama: \texttt{llama3} (8 bilhões de parâmetros), \texttt{Mixtral 8x7B} e \texttt{Gemma 2}. Adicionalmente, o modelo \texttt{llama3.2} (3 bilhões de parâmetros) foi empregado em testes de validação preliminar e experimentos exploratórios acerca do funcionamento das implementações descritas nas seções seguintes, sobretudo devido ao seu menor custo computacional e maior velocidade de execução.
...
Cada uma das estratégias discutidas acima devem criar um texto sintese que reune todas as informações princpais relevantes para o modelo de aa alvo. A partir desse subproduto, entra a fase da passagem das informações não estruturadas para uma forma estruturada
